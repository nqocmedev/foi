...
function showPageTransition(message = 'DevProfile Pro') {
    const transition = document.getElementById('page-transition');
    const loadingText = document.querySelector('.loading-logo');
    
    if (transition && loadingText) {
        loadingText.textContent = message;
        transition.classList.add('active');
    }
}

// Ẩn hiệu ứng chuyển trang
...
// Hiệu ứng chuyển trang cho menu trong trang
function initPageTransitions() {
    // Chọn tất cả link trong menu
    const menuLinks = document.querySelectorAll('nav a.menu-link');
    const mobileMenuLinks = document.querySelectorAll('#mobile-menu a.menu-link');
    
    // Gộp tất cả link
    const allLinks = [...menuLinks, ...mobileMenuLinks];
    
    allLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            const href = this.getAttribute('href');
            
            // Nếu là link đến section trong trang (#)
            if (href.startsWith('#')) {
                e.preventDefault();
                
                // Hiển thị hiệu ứng
                showPageTransition('Đang chuyển trang...');
                
                // Sau 500ms, chuyển đến section
                setTimeout(() => {
                    const targetId = href;
...
// Hiệu ứng chuyển trang cho link đến website khác
function initExternalLinkTransitions() {
    // Chọn tất cả link đến ceotvnqoc.shop
    const storeLinks = document.querySelectorAll('a.store-btn, a[href*="ceotvnqoc.shop"]');
    ...
    storeLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            // Hiển thị hiệu ứng với thông báo khác
            showPageTransition('Đang mở Cửa Hàng...');
            
            // Mở trang mới sau 1 giây
            setTimeout(() => {
                window.open(href, '_blank');
                
                // Reset thông báo và ẩn hiệu ứng
                setTimeout(() => {
                    const loadingText = document.querySelector('.loading-logo');
                    if (loadingText) {
                        loadingText.textContent = 'DevProfile Pro';
                    }
                    hidePageTransition();
                }, 1000);
            }, 800);
...
function initPageLoadEffect() {
    // Tự động ẩn hiệu ứng chuyển trang sau 1 giây
    setTimeout(() => {
        hidePageTransition();
    }, 1000);
    
    // Thêm hiệu ứng cho các card
    const cards = document.querySelectorAll('.bg-white.rounded-2xl, .bg-white.rounded-xl');
...
window.addEventListener('load', initPageLoadEffect);